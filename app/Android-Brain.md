...
- **Kid-Friendly UI Layout**: `SleekTopBar.kt` features a streamlined header with Explorer level and coin balance on top, alongside a single **Activity Hub icon (`🎒`)** that opens a clean bottom sheet containing all sub-features (Trophies 🏆, Daily Quests 📜, Codex 📚, Wardrobe 👗, Room Decor 🛋️, Shop 🛍️, and How to Play Guide 💡). `SleekNeedsDashboard.kt` combines all 5 pet stat indicators (Hunger, Energy, Joy, Clean, Health) and the mood/friendship badge into a single unified card. `CareActionsPanel.kt` normalizes care action cards to a fixed 62dp height with single-line truncated text so all blocks are identical in size. `ShopAndInventorySheet.kt` places the search bar on its own full-width top row with sort chips neatly arranged on the row below. `ic_launcher_foreground.xml` renders the pink-to-lavender gradient Mochi mascot matching in-game appearance. `HowToPlayGuideDialog.kt` provides an interactive guide explaining pet care rooms, mini-games, and activity hub features. `MochiPetView.kt` includes a single-line room title pill to prevent text clipping.
...
5. **Screen 5 (Final Screen)**: Displays "Everything is Ready!" celebration and the "Start Adventure! 🚀" action button.

Completing onboarding calls `viewModel.completeOnboarding(petName)` which sets Level = 1, XP = 0, Coins = 0, updates `isOnboardingCompleted = true` in Room DB, triggers the interactive "How to Play Guide", and seamlessly transitions into the main game.
